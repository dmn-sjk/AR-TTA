...
def get_and_check_domains():
    ...
    if DISCARD_REPEATED_DOMAINS:
        ...
        modified_domains = modified_domains[:-1]
        accepted_domains_idxs = accepted_domains_idxs[:-1]
        
        # modified_domains = modified_domains[1:]
        # accepted_domains_idxs = accepted_domains_idxs[1:]
        
        print(accepted_domains_idxs)
        print(modified_domains)
        # print([i[0] for i in accepted_domains_idxs])
        return modified_domains, accepted_domains_idxs
    else:
        return domains, list(range(len(domains)))
    
def get_run_name_and_method(log):
    with open(os.path.join(LOGS_FOLDER, log, log + '_config.yaml'), "r") as f:
        config = yaml.load(f, Loader=yaml.Loader)
        run_name = config['run_name']
        method = config['method']
    return run_name, method
...
def load_results(accepted_domains_idxs):
    ...
    for log in LOGS_TO_USE:
        ...
        # if 'cifar10c_cifar10c' in log:
        #     log_name = log[9:] + '_results.json'
        # else:
...
def match_same_setups_with_different_seeds():
    ...
    configuration_names = np.unique(run_names_without_seed)
    
    configuration_indices = {}
    for configuration in configuration_names:
        indices = np.where(configuration == run_names_without_seed)[0]
        configuration_indices[configuration] = indices
        
    return configuration_indices

def match_same_domains(domains):
    unique_domains = list(np.unique(domains))
    
    if 'clear' in unique_domains:
        unique_domains.remove('clear')
    elif 'T0' in unique_domains: 
        unique_domains.remove('T0')
    
    domains_arr = np.array(domains)
    # domain_indices = {}
    domain_indices = []
    for domain in unique_domains:
        indices = np.where(domain == domains_arr)[0]
...
def plot_domainwise_acc_train(each_domain_avg_accs_train, domains, args):
    fig, ax = plt.subplots(nrows=1, ncols=1, figsize=(15,10))
    for setup_name, domainwise_avg_accs in each_domain_avg_accs_train.items():
        ax.plot(range(len(domains)), domainwise_avg_accs, marker='.', label=setup_name, color=get_plot_color(setup_name))
...
def plot_plot_per_timeofday_acc(each_domain_avg_accs_train, seed_reduced_per_domain_num_of_batches, domains, args):
    fig, ax = plt.subplots(figsize=(10, 7))
    avg_accs = {'day': [], 'dawn/dusk': [], 'night': []}

    print("\nTime of day results")
    for setup_name, domain_avg_accs in each_domain_avg_accs_train.items():
        sum_day, sum_night, sum_dawndusk, num_day, num_night, num_dawndusk = 0, 0, 0, 0, 0, 0
        for i, (domain, acc, num_of_batches) in enumerate(zip(domains, domain_avg_accs, seed_reduced_per_domain_num_of_batches[setup_name])):
            ...
            if 'day' in domain or any([day_domain in domain for day_domain in clad_day_domains]):
                sum_day += acc * num_of_batches
                num_day += num_of_batches
            ...
            # night for SHIFT, T0, T2, T4 are day sequences of CLAD (old numbering)
                ...
                sum_night += acc * num_of_batches
                num_night += num_of_batches
            
            elif 'dawn/dusk' in domain:
                sum_dawndusk += acc * num_of_batches
                num_dawndusk += num_of_batches

        acc_day, acc_night, acc_dawndusk = 100, 100, 100
        if num_day != 0:
            acc_day = sum_day / num_day
            avg_accs['day'].append(acc_day)
        if num_night != 0:
            acc_night = sum_night / num_night
            avg_accs['night'].append(acc_night)
        if num_dawndusk != 0:
            acc_dawndusk = sum_dawndusk / num_dawndusk
            avg_accs['dawn/dusk'].append(acc_dawndusk)
        
        to_print_acc = f"- {setup_name}: "
        to_print_err = f"  {' ' * len(setup_name)} "
    ...
    min_acc_val = np.min([*avg_accs['day'], *avg_accs['night'], *avg_accs['dawn/dusk']])
    max_acc_val = np.max([*avg_accs['day'], *avg_accs['night'], *avg_accs['dawn/dusk']])
        
    width = 0.25
    plt.xticks(range(len(each_domain_avg_accs_train.keys())), [get_label(setup) for setup in each_domain_avg_accs_train.keys()], rotation=0)
    # plt.title(f"Avg train accuracy time of day")
    if len(avg_accs['dawn/dusk']) > 0:
        ax.bar(np.array(range(len(each_domain_avg_accs_train.keys()))) - width, avg_accs['day'], color='gold', width=width, label='day')
        ax.bar(np.array(range(len(each_domain_avg_accs_train.keys()))), avg_accs['dawn/dusk'], color='orange', width=width, label='dawn/dusk')
        ax.bar(np.array(range(len(each_domain_avg_accs_train.keys()))) + width, avg_accs['night'], color='darkblue', width=width, label='night')
    else:
        ax.bar(np.array(range(len(each_domain_avg_accs_train.keys()))) - (width / 2), avg_accs['day'], color='gold', width=width, label='day')
        ax.bar(np.array(range(len(each_domain_avg_accs_train.keys()))) + (width / 2), avg_accs['night'], color='darkblue', width=width, label='night')
    ax.set_xlabel("Method", fontdict=font)
    ax.set_ylabel("Accuracy [%]", fontdict=font)
    ax.set_ylim(min_acc_val - 5, max_acc_val + 5)
...
def plot_per_domain_change(each_domain_avg_accs_train, domains_indices, args):
    pad = len(max(domains_indices, key=len))
    domains_indices_padded = np.array([list(i) + [None]*(pad-len(i)) for i in domains_indices])
    
    fig, ax = plt.subplots(figsize=(10, 7))
    
    for method, values_sequence in each_domain_avg_accs_train.items():
        occurances_sequnce_mean_diff_accs = []
        
        first_occurance_indices = domains_indices_padded[:, 0].astype(np.uint8)
        firt_accs = values_sequence[first_occurance_indices]
        
        for occurance_num in range(1, domains_indices_padded.shape[1]):
            occurance_indices = domains_indices_padded[:, occurance_num]
            

            # filter domains that does not occur anymore
            occurance_indices_wo_none = occurance_indices[occurance_indices != None].astype(np.uint8)
            
            occ_accs = values_sequence[occurance_indices_wo_none]
            
            diff = np.mean(occ_accs - firt_accs[occurance_indices != None])
            occurances_sequnce_mean_diff_accs.append(diff)

        ax.plot(range(domains_indices_padded.shape[1] - 1), occurances_sequnce_mean_diff_accs, label=method)
...
def main(args):
    ...
    for i, method in enumerate(LOGS_TO_USE):
        ...
        for j, domain_name in enumerate(domains):
            ...
            per_domain_num_of_batches[i].append(len(single_domain))

            if not (NOT_INCLUDE_LAST_DOMAIN_IN_AVERAGE and j == len(domains) - 1):
    ...
    seed_avg_table = [["method", *domains, "Avg"], *[[] for i in range(len(matched_setups.keys()))]]
    results_arr = np.array(table[1:])[:, 1:].astype(np.float32)
    seed_avg_accs_train = {}
    each_domain_avg_accs_train = {}
    seed_reduced_per_domain_num_of_batches = {}
    
    for i, (setup_name, setup_idxs) in enumerate(matched_setups.items()):
        seed_avg_table[i + 1].append(setup_name)
        seed_avg_results = np.mean(results_arr[setup_idxs], axis=0)
        seed_avg_table[i + 1].extend(seed_avg_results)

        # avg result of whole domain is the last one in the table
        seed_avg_accs_train[setup_name] = seed_avg_results[-1]
        each_domain_avg_accs_train[setup_name] = seed_avg_results[:-1]
        seed_reduced_per_domain_num_of_batches[setup_name] = per_domain_num_of_batches[setup_idxs[0]]

        print(f'- {setup_name}: \tAcc: {seed_avg_accs_train[setup_name]:.2f}, \tClassification error: {100 - seed_avg_accs_train[setup_name]:.2f}')

    if args.save_results:
        with open(os.path.join(RESULTS_FOLDER, args.results_name, args.results_name + '.csv'), 'w') as f:
            writer = csv.writer(f)
            for row in seed_avg_table:
                writer.writerow(row)
    ...
    # if 'shift' in LOGS_TO_USE[0] or 'clad' in LOGS_TO_USE[0]:
        # per_time_of_day_accs = plot_plot_per_timeofday_acc(each_domain_avg_accs_train, seed_reduced_per_domain_num_of_batches, domains, args)
    # =======================================================================================
    # plot_per_domain_change(each_domain_avg_accs_train, domains_indices, args)
