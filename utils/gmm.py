...
class GaussianMixture(torch.nn.Module):
    ...
    def _m_step(self, x, log_resp):
        ...
        if self.covariance_type == "full":
            ...
            resp = resp.to(torch.float16) 
            
            # print("before summing")
            summing = torch.sum((x - mu).unsqueeze(-1).matmul((x - mu).unsqueeze(-2)) * resp.unsqueeze(-1), dim=0,
                            keepdim=True, dtype=torch.float16)
            # print("after summing")
            div = torch.sum(resp, dim=0, keepdim=True, dtype=torch.float16).unsqueeze(-1)
            # print("after div")
            var = summing / div + (torch.eye(self.n_features, dtype=torch.float16) * self.eps).to(x.device)
            # print("after full")
