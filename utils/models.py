...
def get_model(cfg):
    if cfg['model'] == 'resnet18':
        ...
        if cfg['dataset'] == 'imagenetc':
            model = load_model('Standard_R50', cfg['ckpts_dir'],
        ...
        model = load_model('Standard', cfg['ckpts_dir'],
                            dataset, "corruptions")
    ...
    if cfg['src_model_ckpt_file'] is not None:
        src_model_path = os.path.join(cfg['ckpts_dir'], cfg['src_model_ckpt_file'])
        model.load_state_dict(torch.load(src_model_path))
